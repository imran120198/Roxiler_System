...
DataRouter.get("/transactions", async (req, res) => {
  try {
    ...
    const data = await TransactionModel.find(searchConfig)
      .skip(skip)
      .limit(limit);
    const totalCount = await TransactionModel.countDocuments(searchConfig);
...
DataRouter.get("/statistics", async (req, res) => {
  try {
    const month = !isNaN(parseInt(req.query.month))
      ? parseInt(req.query.month)
      : 3;
    ...
    const data = await TransactionModel.find(monthQuery, projectQuery);
...
DataRouter.get("/bar-chart", async (req, res) => {
  try {
    const month = !isNaN(parseInt(req.query.month))
      ? parseInt(req.query.month)
      : 3;
...
DataRouter.get("/pie-chart", async (req, res) => {
  try {
    const month = !isNaN(parseInt(req.query.month))
      ? parseInt(req.query.month)
      : 3;
...
DataRouter.get("/combined-data", async (req, res) => {
  try {
